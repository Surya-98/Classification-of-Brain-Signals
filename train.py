    ...
    for i1 in range(14):
        ...
        for i2 in range(len(L[0][i1])):
            for i3 in range(0, len(L[0][i1][i2])-window_size, overlap):
                if (i1<10 and i1!=1):
                    x_train.append(preprocess(L[0][i1][i2][i3:i3+window_size]))
                    y_train.append(L[1][i1][i2])
                else:
...
print(y_train)
model.fit(x_train, y_train, epochs=10, shuffle=True, validation_data=[x_test, y_test],callbacks=[checkpointer])
